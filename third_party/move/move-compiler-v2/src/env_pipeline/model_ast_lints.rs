...
trait ExpressionLinter {
    ...
    fn warning(&self, env: &GlobalEnv, loc: &Loc, msg: &str) {
        env.lint_diag_with_notes(loc, msg, vec![
            ...
            "However, note that such an annotation will also suppress other warnings of the same kind in the function/module.".to_string(),
            "It is recommended to fix the issue instead of suppressing the warning.".to_string(),
        ]);
    }
}
